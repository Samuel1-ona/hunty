...
- `scripts/verify-email-digest.sh` - Automated deployment verification script
...
ADMIN_TOKEN=test-token ./scripts/verify-email-digest.sh
```
...
5. Verify with script:
   ```bash
   ./scripts/verify-email-digest.sh
   ```
...
- Run `scripts/verify-email-digest.sh` after deployment
