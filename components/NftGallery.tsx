      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" size="sm">{selectedHunt === "All" ? "All Hunts" : selectedHunt}</Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent className="w-48">
            <DropdownMenuItem onClick={() => setSelectedHunt("All")}>All Hunts</DropdownMenuItem>
            {huntOptions.map((hunt) => (
              <DropdownMenuItem key={hunt} onClick={() => setSelectedHunt(hunt)}>{hunt}</DropdownMenuItem>
            ))}
          </DropdownMenuContent>
        </DropdownMenu>
        {/* Date filter */}
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" size="sm">Sort: {sortOption === "newest" ? "Newest" : "Rarest"}</Button>
          </DropdownMenuTrigger>
        ...
        {displayedNfts.map((nft, idx) => (
          ...
          >
            <Card className={cn(
              ...
              <CardContent className={cn("p-0", viewMode === "list" && "flex items-center p-4")}>
                {/* Badge */}
                ...
                <div className={cn(
                  ...
                  viewMode === "list" && "w-24 h-24 flex-shrink-0 mr-4"
                )}>
