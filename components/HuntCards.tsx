...
export const HuntCards: React.FC<HuntCardsProps> = ({
  ...
  const ownCount = usePlayerCount(
    playerCountProp !== undefined ? "" : fallbackId,
  );

  const count =
    playerCountProp !== undefined ? playerCountProp : ownCount.count;
  const countIsLoading =
    playerCountProp !== undefined
      ? (playerCountLoadingProp ?? false)
      : ownCount.isLoading;
  const countError =
    playerCountProp !== undefined
      ? (playerCountErrorProp ?? null)
      : ownCount.error;
  const trending =
    playerCountProp !== undefined
      ? (isTrendingProp ?? false)
      : ownCount.isTrending;
  ...
  const handleUnlock = async () => {
    ...
    if (huntId != null) {
      ...
      if (
        input.trim().toLowerCase() === (hunt.code || "").trim().toLowerCase()
      ) {
        setSuccess(true);
    ...
    return (
      <div
        className={cn(
          ...
          isActive
            ? "sm:scale-105 border-2 border-blue-400"
            : preview
              ? "opacity-70"
              : "opacity-90",
        )}
    ...
    >
            ...
            <span
              className={cn(
                ...
                hunt.difficulty === "Hard" &&
                  "bg-red-500/30 text-red-200 print:border-red-500",
              )}
            >
              {hunt.difficulty}
            </span>
        ...
        <div className="bg-white dark:bg-slate-900 px-4 sm:px-6 py-2 border-b border-gray-100 dark:border-white/5 print:hidden">
          {!hintRevealed ? (
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 text-blue-800 dark:text-blue-300 p-2 sm:p-3 rounded-lg sm:rounded-xl text-xs sm:text-sm border border-blue-100 dark:border-blue-900/30">
              <span className="font-semibold text-blue-900 dark:text-blue-200 mr-2">
                Hint:
              </span>
              <span
                dangerouslySetInnerHTML={{
                  __html: sanitizeHtml(hunt.hint || ""),
                }}
              />
            </div>
          )}
      ...
      <div className="bg-white dark:bg-slate-900 rounded-b-xl sm:rounded-b-2xl -mt-4 pb-4 px-4 sm:px-6 min-h-[36px] print:hidden">
        ...
        {!huntEnded && !success && isPending && (
          <p className="text-center text-slate-400 dark:text-slate-500 text-xs sm:text-sm">
            Submitting...
          </p>
        )}
        {!huntEnded && !success && !isPending && error && (
          <p className="text-center text-red-500 dark:text-red-400 font-semibold text-xs sm:text-sm">
            {error}
          </p>
        )}
      </div>
