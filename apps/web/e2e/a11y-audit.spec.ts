...
function saveReport(pageName: string, results: Awaited<ReturnType<AxeBuilder['analyze']>>): void {
...
async function auditPage(page: Page, pageName: string): Promise<void> {
  ...
  const newViolations = blockingViolations.filter((v) => !baselineBlockingIds.has(v.id));
  ...
  const baselineBlockingCount = baselineRecord
    ? (baselineRecord.violations ?? []).filter((v) => v.impact && BLOCKING_IMPACTS.has(v.impact))
        .length
    : 0;
...
test.describe('Axe Accessibility Audits', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.addInitScript(() => {
      const mockKey = 'GAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA1';
      ...
      localStorage.setItem(
        'hunty_clues',
        JSON.stringify([{ id: 1, huntId: 100, question: 'What is 2+2?', answer: '4', points: 10 }])
      );
    });
  });
  ...
  test('landing page meets WCAG 2.1 AA (with baseline burn-down)', async ({ page }) => {
    await page.goto('/');
  ...
  test('hunt detail page meets WCAG 2.1 AA (with baseline burn-down)', async ({ page }) => {
  ...
  test('play/create page meets WCAG 2.1 AA (with baseline burn-down)', async ({ page }) => {
  ...
  test('leaderboard page meets WCAG 2.1 AA (with baseline burn-down)', async ({ page }) => {
  ...
  test('hunt leaderboard page meets WCAG 2.1 AA (with baseline burn-down)', async ({ page }) => {
