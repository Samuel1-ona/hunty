...
function createMockWallet(overrides?: {
  ...
  return {
    ...
    getPublicKey: vi
      .fn()
      .mockResolvedValue('GTEST0000000000000000000000000000000000000000000000000000'),
...
describe('readContract', () => {
  ...
  it('throws when RPC returns an error', async () => {
    ...
    await expect(readContract({ contractId: 'C...', method: 'bad_method' })).rejects.toMatchObject({
  ...
  it('throws when HTTP request fails', async () => {
    ...
    await expect(readContract({ contractId: 'C...', method: 'read_method' })).rejects.toMatchObject(
      {
