...
export async function dbUpsertCollaborator(
  huntId: number,
  collaborator: HuntCollaborator
): Promise<void> {
  const sql = getDb();
  await sql`
    INSERT INTO hunt_collaborators (hunt_id, wallet_address, role, invited_at, invited_by, accepted, last_active_at, editing_field)
...
export async function dbPingPresence(
  huntId: number,
...
export async function dbGetActiveEditors(
  huntId: number,
  excludeAddress?: string,
  staleMs = 30_000
): Promise<HuntCollaborator[]> {
  const sql = getDb();
  const nowMs = Date.now();
...
export async function dbGetRoleForWallet(
  huntId: number,
  walletAddress: string
...
export async function dbEnsureOwner(
  huntId: number,
  ownerAddress: string
): Promise<HuntCollaborator> {
  const sql = getDb();
...
export async function dbInviteCollaborator(
  ...
  if (!inviter[0] || (inviter[0].role !== "owner" && inviter[0].role !== "editor")) {
    const count =
      await sql`SELECT COUNT(*) as cnt FROM hunt_collaborators WHERE hunt_id = ${huntId}`;
