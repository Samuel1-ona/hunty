...
import { attachMediaTypeToCid, getClueMediaKind, getClueMediaSource } from '@/lib/clueMedia';

describe('clueMedia', () => {
  it('attaches the top-level mime type to the media URI', () => {
    expect(attachMediaTypeToCid('ipfs://bafy123', 'audio/mpeg')).toBe('ipfs://bafy123?type=audio');
  });
