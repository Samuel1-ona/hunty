...
const mockGetPublicKey = vi
  .fn()
  .mockResolvedValue('GCREATOR0000000000000000000000000000000000000000000000000');
...
vi.mock('@/lib/contracts/config', () => ({
  getRequiredRewardManagerAddress: () =>
    'GREWARDMANAGER0000000000000000000000000000000000000000000',
...
describe('refundUnclaimedRewards', () => {
  ...
  it('throws when there is no remaining balance to refund', async () => {
    ...
    const { refundUnclaimedRewards } = await import('../rewardManager');
    await expect(refundUnclaimedRewards(HUNT_ID)).rejects.toThrow('No unclaimed rewards remain');
  });
  ...
  it('throws when the blockchain transaction returns no hash', async () => {
    ...
    const { refundUnclaimedRewards } = await import('../rewardManager');
    await expect(refundUnclaimedRewards(HUNT_ID)).rejects.toThrow('Reward transaction failed');
  });
