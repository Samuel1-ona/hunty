...
vi.mock('@/lib/walletAdapter', () => ({
  getActiveWalletAdapter: vi.fn(() => ({
    getPublicKey: vi
      .fn()
      .mockResolvedValue('GCREATOR1AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA'),
...
vi.mock('@/lib/contracts/config', () => ({
  getRequiredRewardManagerAddress: vi.fn(
    () => 'GREWARD_MANAGER_ADDRESS_00000000000000000000000000000000000'
  ),
...
describe('#1173 refundUnclaimedRewards', () => {
  describe('grace period enforcement', () => {
    it('throws when called before the grace period has elapsed', async () => {
      ...
      await expect(rm.refundUnclaimedRewards(huntId, CREATOR, 0)).rejects.toThrow(
        /Grace period has not yet elapsed|grace period|expires/
      );
    });
    ...
    it('throws when the escrow has expired but the grace period has not', async () => {
      ...
      await expect(rm.refundUnclaimedRewards(huntId, CREATOR, 3600)).rejects.toThrow(
  ...
  describe('creator authorisation', () => {
    it('throws when a non-creator address attempts a refund', async () => {
      ...
      await expect(rm.refundUnclaimedRewards(huntId, SPONSOR, 0)).rejects.toThrow(
        /Only the hunt creator/
      );
    });
  ...
  describe('escrow state after refund', () => {
    ...
    it('throws when no escrow exists for the hunt', async () => {
      const rm = await freshModule();
      await expect(rm.refundUnclaimedRewards(999, CREATOR, 0)).rejects.toThrow(/No reward escrow/);
    });
  });
