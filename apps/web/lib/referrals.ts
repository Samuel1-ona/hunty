...
export function consumePendingReferral(referredAddress: string): ReferralRecord | null {
  if (typeof window === 'undefined') return null;
...
export function awardReferralBonusOnFirstCompletion(
  ...
  const index = records.findIndex((record) => record.referredAddress === normalizedReferred);
