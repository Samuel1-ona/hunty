...
import { Flame, Sparkles, MapPin, Star, Compass, RefreshCw, Search } from 'lucide-react';
...
> = {
  trending: {
    ...
    description:
      'Be the first to play and make a hunt trend! Create or join a hunt to get things started.',
  ...
  nearby: {
    ...
    description:
      'There are no hunts near your location. Enable location access or check other categories.',
...
function getHuntDistanceMeters(
  hunt: StoredHunt,
...
export function HuntFeed({
  ...
  const activeCategoryInfo = CATEGORIES.find((cat) => cat.key === activeCategory);
  ...
  return (
    <section aria-label="Hunt Feed" className={cn('w-full', className)}>
      ...
      {showHeader && (
        <div className="mb-6">
          ...
          {/* Geo error message */}
          {geoError && (
            <p className="mt-2 text-xs text-amber-600 dark:text-amber-400 px-1">{geoError}</p>
          )}
        </div>
      )}
      ...
      >
        {/* Pull-to-refresh indicator */}
        <PullToRefreshIndicator refreshing={isRefreshing} pullDistance={pullDistance} />
