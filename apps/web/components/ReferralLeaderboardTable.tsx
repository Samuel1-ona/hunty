...
function SkeletonRow() {
  return (
    <tr className="animate-pulse">
      ...
      <td className="px-4 py-3 text-center">
        <div className="h-4 w-12 rounded bg-white/10 mx-auto" />
      </td>
      <td className="px-4 py-3 text-center">
        <div className="h-5 w-16 rounded-full bg-white/10 mx-auto" />
      </td>
    </tr>
  );
}
    ...
    <section aria-label="Referral leaderboard" data-testid="referral-leaderboard">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        <div>
          ...
          <p className="text-zinc-400 text-sm mt-0.5">Top players ranked by successful referrals</p>
        </div>
        <div
          ...
          {PERIOD_OPTIONS.map((opt) => (
            <button
              ...
              className={`px-4 py-1.5 text-xs font-medium transition-colors ${
                period === opt.value ? 'bg-violet-600 text-white' : 'text-zinc-400 hover:text-white'
              }`}
            >
              {opt.label}
            </button>
      ...
      {/* Stats + reward pool */}
      {data && !loading && <StatsCard stats={data.stats} rewardPool={DEFAULT_REWARD_POOL} />}
                ...
                return (
                  <tr
                    ...
                    className={`border-b border-white/5 transition-colors ${
                      isCurrentPlayer ? 'bg-violet-900/30 border-violet-600/30' : 'hover:bg-white/5'
                    }`}
                  >
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-center text-zinc-300">{entry.totalInvites}</td>
                    <td className="px-4 py-3 text-center text-violet-300 font-semibold">
                      {entry.bonusPoints}
                    </td>
                    <td className="px-4 py-3 text-center">
                      <PayoutBadge status={entry.rewardPayoutStatus} amount={entry.rewardAmount} />
                    </td>
                  </tr>
                );
              })
