...
- `scripts/verify-email-digest.sh` - Deployment verification script
...
4. **Test locally:**
   ...
   ```bash
   chmod +x scripts/verify-email-digest.sh
   ADMIN_TOKEN=your-token ./scripts/verify-email-digest.sh
   ```
...
See `scripts/verify-email-digest.sh` for automated testing.
...
- Run `scripts/verify-email-digest.sh` to diagnose
